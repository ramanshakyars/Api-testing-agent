...
import httpx
import json
from crewai import Agent, Task, Crew, LLM  # ✅ Correct import (moved from crewai_tools)
...
if st.button("2. Start Testing") and base_url:
    ...
    predefined_testcases = [
        {"name": "Create new chat message", "method": "POST", "endpoint": "/chat/message",
         "data": {"prompt": "Hello from test"}, "expected_status": [200]},
        {"name": "Get chat history", "method": "GET", "endpoint": "/chat/history", "expected_status": [200]},
        {"name": "Load chat by ID", "method": "GET", "endpoint": "/chat/{chatId}", "chatId": "123",
         "expected_status": [200, 404]},
        {"name": "Rename chat", "method": "PUT", "endpoint": "/chat/rename/{chatId}",
         "data": {"title": "Renamed Chat"}, "chatId": "123", "expected_status": [200, 404]},
        {"name": "Delete chat", "method": "DELETE", "endpoint": "/chat/{chatId}", "chatId": "123",
         "expected_status": [200, 404]}
    ]
    ...
    testcase_task = Task(
        ...
        agent=tester_agent,
        context=[plan_task],
        expected_output="JSON string of testcases"
    )

    report_task = Task(
        description=f"Summarize the final API test results from the logs into a JSON report. The test execution logs are: {json.dumps(predefined_testcases)}. The final JSON report must include a 'summary_status' (e.g., PASSED, FAILED) and the 'execution_logs' array.",
